...
- Spaces can lend you a browser and the screen, when your agent's reach is switched on. \`spaces_browse\` opens an address in the workspace browser — the one the person can see — and reads the page back; \`spaces_browser_read\`, \`spaces_browser_controls\`, \`spaces_browser_click\` and \`spaces_browser_type\` work it, addressed by the text a person would read rather than by CSS selectors. \`spaces_screen_read\` reads another application's window as text and \`spaces_screen_type\` types into one. If a tool answers that reach is off, say which switch the person needs to turn on rather than working around it. Spaces will not type into a password field, and will not sign you in.
- Your harness's *own* GUI Computer Use approval is a different thing, and belongs to the host harness task rather than to Spaces or macOS. A headless channel run cannot display that approval UI, so never ask someone to approve your harness's computer use in Spaces. For Apple development and capture, prefer the non-GUI path: \`xcodebuild\` to build, \`xcrun simctl\` to boot/install/launch, and \`xcrun simctl io booted recordVideo <project-path>\` to record.
