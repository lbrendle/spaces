...
export type KnowledgeTreeNode<T> =
  | KnowledgeTreeFolder<T>
  | KnowledgeTreeNote<T>;
...
function compareNodes<T>(
  left: KnowledgeTreeNode<T>,
  right: KnowledgeTreeNode<T>,
): number {
...
/**
 * Reconstruct a deterministic vault tree from durable source/path values.
 * Member-local mirror IDs are intentionally absent from folder identity.
 */
export function buildKnowledgeTree<T>(
  ...
  for (const entry of entries) {
    const sourceName = entry.sourceLabel.trim() || "Workspace knowledge";
