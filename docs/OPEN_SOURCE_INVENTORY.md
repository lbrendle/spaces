...
| CLI coding agents (Claude Code, Codex, Cursor Agent, and others) | Implemented | Local process/session state | An open harness registry: each entry declares its transport, launch shape, stream grammar, resume support and MCP wiring. A versioned Spaces event-harness contract routes explicit channel/event/reply context into one resumable session per `(channel, agent)`. Each user authenticates the CLI they run; Spaces stores no provider API key. Gemini CLI, Aider and OpenCode ship as presets whose flags Spaces has not verified, and the editor says so. |
