...
| CLI coding agents (Claude Code, Codex, Cursor Agent) | Implemented | Local process/session state | A curated harness registry: each entry declares its transport, launch shape, stream grammar, resume support and MCP wiring, and each is verified against that harness's own `--help`. There is no generic "run this executable" kind; an unrecognised one is treated as an agent Spaces will not launch. A versioned Spaces event-harness contract routes explicit channel/event/reply context into one resumable session per `(channel, agent)`. Each user authenticates the CLI they run; Spaces stores no provider API key. |
| One-click agent setup | Implemented | Local only | A card per supported agent showing whether it can run from this Mac — installed, signed out, missing, or unverifiable — and creating a configured teammate in one click, including the settings that are not discoverable from a form (an app's bundle identifier, a CLI's real binary name). |
| External (GUI) agents, e.g. Muse | Implemented | Local git and filesystem | Spaces never launches these — they run in their own app against the same checkout. They appear in the roster, the branch lanes, the overlap check and every other agent's prompt; addressing one writes a hand-off brief, and its result is read back out of git rather than reported by hand. |
| Shared-workspace coordination | Implemented | Local git only | Files more than one agent is changing, ranked so a live collision in one tree outranks a future merge conflict; a landing order simulated with `git merge-tree` in memory, which sees branches that conflict with each other and not only with the base; one-at-a-time landing with strict preconditions. |
| Harness doctor | Implemented | Local only | Per-harness: binary on PATH, version, sign-in where a verified command exists, and installed/running for a GUI app. An unverifiable sign-in state is reported as unknown rather than assumed good. |
...
| Spaces event harness and agent tools | Implemented | Durable SQLite event queue/session map, live paired-host reads, generated `.hq/` fallbacks, and the desktop approval log | Code projects use their checkout. Non-code projects receive a private control directory so tools do not disappear. Each turn carries an authoritative `[Spaces Context]` envelope and matching `SPACES_*` environment. Agents can read current channel history and natively list/read shared documents, private mail, calendars, Content Studio, connected social routing, Knowledge references, and Git state; they can create/update governed records and open the persistent project browser. MCP is primary, the dependency-free server also supports one-shot structured CLI calls, and `.hq/actions.jsonl` remains the universal write fallback. |
| Agent Content Studio tools and social publishing | Implemented | Shared D1 Content Studio + paired-host live reads + `.hq/CONTENT.md` fallback + desktop approval log + portal provider action | Agents can list, read, create, fully revise, stage, deduplicate, and publish canonical board cards through `spaces_list_content`, `spaces_get_content`, `spaces_create_content`, `spaces_update_content`, `spaces_delete_content`, and `spaces_publish_social`. `spaces_list_social_accounts` exposes the exact linked/default project account first. Publishing and deletion wait for human approval. Local media can be uploaded through `media_path`. Existing agent sessions must restart to discover newly added tools. |
