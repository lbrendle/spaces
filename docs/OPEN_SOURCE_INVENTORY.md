...
| Spaces agent MCP tools | Implemented | Live paired-host SQLite reads, generated `.hq/` fallbacks, and the desktop approval log | Code projects use their checkout. Non-code projects receive a private control directory so tools do not disappear. Spawned harnesses receive explicit run, agent, channel, project, and local database identity. Agents can natively list/read shared documents, private mail, calendars, Content Studio, connected social routing, Knowledge references, and Git state; they can also create/update governed records and open the persistent project browser. |
