...
| Claude Code and Codex agents | Implemented | Local process/session state | A versioned Spaces event-harness contract routes explicit channel/event/reply context into one resumable session per `(channel, agent)`. Each user authenticates the CLI they run; Spaces stores no Claude/Codex API key. |
